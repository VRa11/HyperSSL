...
from model import GCN_mgaev2 as GCN_mgae 
...
def MGAE(data_name,args):         
    ...
    save_path_model = 'MGAE'+ '_model.pth'
    save_path_predictor = 'MGAE'+'_pred.pth'
    ...
    model = GCN_mgae(data.num_features, args.hidden_channels,
                         args.hidden_channels, args.num_layers,
                         args.dropout, decoder_mask=args.decoder_mask, num_nodes=data.num_nodes).to(device)               
    ...
    print(f"##### Final Testing result- {data_name} MGAE ")
    for key in loggers.keys():        
        loggers[key].print_statistics(key)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='MGAE')
