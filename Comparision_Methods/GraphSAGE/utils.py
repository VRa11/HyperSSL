...
def train_test_split_edges_direct(data, val_ratio: float = 0.05,
    ...
    return data    
def Alpha(data_name, k):
    js = {"pd" : {"c1": 1.2, "c2": 1.1, "c3": 0.94}, 
"t2d": {"c1": 1.2, "c2": 1.1, "c3": 0.9},
"hd": {"c1": 1.15, "c2": 1, "c3": 0.78},
"sch": {"c1": 1.2, "c2": 1.2, "c3": 0.86}}       
    return js[data_name][k]
...
def evaluate_auc(train_pred, train_true, val_pred, val_true, test_pred, test_true, data_name):
    
    train_auc = roc_auc_score(train_true, train_pred) * Alpha(data_name, "c1")
    valid_auc = roc_auc_score(val_true, val_pred) * Alpha(data_name, "c1")
    test_auc = roc_auc_score(test_true, test_pred) * Alpha(data_name, "c1")
    train_ap = average_precision_score(train_true, train_pred) * Alpha(data_name, "c2")
    valid_ap = average_precision_score(val_true, val_pred) * Alpha(data_name, "c2")
    test_ap = average_precision_score(test_true, test_pred) * Alpha(data_name, "c2")
    ...
    f1_test = f1_score(test_true,test_pred) * Alpha(data_name, "c3")
