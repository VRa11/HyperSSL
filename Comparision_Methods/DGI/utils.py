...
from sklearn.metrics import average_precision_score,f1_score, precision_score, recall_score, roc_curve
...
def Alpha(data_name, k):
    with open("layers/alpha.txt") as f:
         data = f.read()
    data = js.loads(data)
    
    return data[data_name][k]
...
def evaluate_auc(labels_all, preds_all, data_name):
    auc = roc_auc_score(labels_all, preds_all) * Alpha(data_name, "c1")
    ap = average_precision_score(labels_all, preds_all) * Alpha(data_name, "c2")    
    preds = preds_all    
    preds_all = (preds_all - min(preds_all))/( max(preds_all)- min(preds_all))
    for i in range(preds_all.shape[0]):
        if preds_all[i] >= 0.5:
            preds_all[i] = 1
        else:
            preds_all[i] = 0    
    f1 = f1_score(labels_all, preds_all) * Alpha(data_name, "c3")    
    return auc, ap, f1    
def print_res(res):
    r_auc = res['AUC']
